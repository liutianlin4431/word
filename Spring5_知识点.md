...
<img src="20160417164310654.png" style="zoom: 80%;" />
...
1. @Before：前置通知
2. @After：最终通知（方法执行之后执行）
3. @AfterReturning：后置通知（返回值之后执行；当有异常时不执行该通知）
4. @AfterThrowing：异常通知（异常通知触发后）
5. @Around：环绕通知
...
```
1、propagation:传播行为
	（1）多事务方法（对数据库表数据进行变化得操作）进行调用，这个过程中事务是如何进行管理得
2、ioslation:隔离级别
3、timeout:超时时间
4、readOnly:是否只读
6、rollbackFor:回滚
...
```tex
1、@Nullable注解可以使用在方法，属性，参数上面表达方法返回、属性值，参数值可以为空
```
